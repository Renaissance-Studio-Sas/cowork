// In-process MCP server for email preview and approval.
// Allows agents to compose emails that users can review before sending.
// One instance per session — it closes over the sessionId so the agent can
// compose emails and check approval status without specifying session context.
...
export function buildEmailMcp(sessionId: string): McpSdkServerConfigWithInstance {
  return createSdkMcpServer({
    name: "workbench-email",
    version: "0.1.0",
    tools: [
      tool(
